...
const HomeSearchFormContainer = () => {
  ...
  const fetchResults = async () => {
    ...
    setError(null);
    try {
      const response = await axios.get('https://rodex-server.vercel.app/api/music/tracks/admin', {
  ...
  return (
    <React.Fragment>
      ...
      {error && <div className="search-results search-result-three">{error}</div>}
      {searchTerm && searchResults.length === 0 && !isSongLoading && (
        <div className="search-results search-result-three">No results found</div>
      )}
      {searchResults.length > 0 && (
        <div className="search-results search-result-three" ref={searchRef}>
