...
import ArtistPlaylist from './artistSongsPlaylist';
...
const ArtistSingle = () => {
  const { id } = useParams();
  const { setId } = useArtistId(); // Get the setter function from the context
  ...
  const { artistSongsPlaylist: songs, isPlaylistSongLoading, isError: songsError } = ArtistPlaylist(id);
  ...
  useEffect(() => {
    setIsLoading(true);
    axios.get(`https://rodex-server.vercel.app/api/artist/getArtists/`, { headers: { Authorization: `Bearer ${rodexAppToken}` } })
      .then(response => {
        const mainArtist = response.data.find(artist => artist._id === id);
  ...
  useEffect(() => {
    if (id) {
      setId(id); // Set the artistId in the context
    }
  }, [id, setId]);

  useEffect(() => {
    if (songs && !isPlaylistSongLoading) {
      setFilteredSongs(songs.slice(0, 10));
    }
  }, [songs, isPlaylistSongLoading]);
  ...
  return (
    <React.Fragment>
      <div className="home-container home-container-image" style={{backgroundImage: `url(${artist.image})`, backgroundSize: 'cover'}}>
        <div className="home-container-image-overlay">
          <div className="home-container-col lg-home-container">
            ...
            <div className="home-welcome-box wow fadeIn rodex-hw" data-wow-duration='0.8s'>    
              <div className="album-cover">
                <div className="album-cover-imgbox">
                  <img src={artist.image || Album_Image} alt="album-img" className="album-cimg"/>    
                </div>
                <div className="album-cover-textbox">
                  <div className="album-cover-artist">{artist.artist}</div>    
                  <div className="album-cover-description">{artist.biography}</div>    
                  <div className="album-cover-title">{artist.title}</div>    
                </div>
              </div>    
            </div> 
            ...
            {isLoading || isPlaylistSongLoading ?
