...
const CuratedPlaylistSongs = () => {
  const { curatedId } = useParams();
      ...
      try {
        ...
        if (adminUser) {
          ...
          if (curatedPlaylist) {
            ...
            const playlistNamer = curatedPlaylist.title;
            const curatedTracks = curatedPlaylist.tracks.reverse();
            setAllSongs(curatedTracks);
            setPlaylistsName(playlistNamer);
            setDisplayedSongs(curatedTracks.slice(0, 10));
            setPlaylistsData(curatedPlaylist);

          } else {
            setDisplayedSongs([]); // Or handle the case where no playlist matches the curatedId
          }
          setIsError(false);
        }
      } catch (error) {
  ...
  const handleSongClick = (song) => {
    ...
    setcPlaylistId(playlistData?._id);
...
export default CuratedPlaylistSongs;
