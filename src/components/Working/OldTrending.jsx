...
const Trending = () => {
  ...
  const songsToUse = token ? playlistSongs : localPlaylistSongs;
  ...
  }, [playlistSongs, isLoading]);



  useEffect(() => {
    if (!isLoading && playlistSongs) {
  ...
  function filterSongs(genre) {
    const filtered = playlistSongs.filter(song => genre === "All Genres" || song.genre === genre);
