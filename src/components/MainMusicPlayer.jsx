...
const MainMusicPlayer = ({ showMusicPlayer, toggleShow }) => {
  ...
  useEffect(() => {
    const updatePlayerTime = () => {
      ...
      const inputElement = document.getElementById('new-progress');
      if (inputElement) {
  ...
  useEffect(() => {
    const input = document.getElementById('new-progress');
    if (input) {
  ...
  useEffect(() => {
    const handlePopState = (event) => {
      if (showMusicPlayer) {
        toggleShow();
        event.stopImmediatePropagation(); // Prevent the event from affecting the route
      }
    };
  ...
  }, [showMusicPlayer, toggleShow]);



    return (
    <React.Fragment>
    <div className={`main-music-player ${showMusicPlayer && 'show-music-player'}`}>

        <img src={currentSong?.image || Music_Pix} className='md-main-box-img' alt='album-img'/>

        <div className='sec-music-player'>

        <div className='fas fa-angle-left close-music-player' onClick={toggleShow}></div>

        <div className='music-display-box'>

                {currentSong
                ?
                <div className='md-box-two'>
                    <div className={`md-box-three ${isPlaying && 'md-spin'}`}>
                        <img src={currentSong?.image || Music_Pix} className='md-box-img' alt='album-img'/>
                        <div className='md-box-line'></div>
                    </div>
                </div>
                :
                <div className='md-box-two'>
                    <div className='md-box-three'>
                        <div className='md-box-four'>
                            <div className='md-box-five'></div>
                        </div>
                    </div>
                </div>
                }
        ...
        <div className='mdsag-box'>

          <div className='md-song-title'>{currentSong?.title || 'Song Title'}</div>
          <div className='md-song-artist'>{currentSong?.artist || 'Artist'}</div>
          <div className='md-song-genre'>{currentSong?.genre || 'Genre'}</div>

        </div>

        
        <div className='md-ranger-box'>
            <div className='md-range-start'>{formatTime(currentTime)}</div>
            <div className="md-range-box-hold">
            <input type="range" value={currentTime} max={duration} id="new-progress" className="range-input" onChange={handleChange} />
            </div>
            <div className='md-range-stop'>{formatTime(duration)}</div>
        </div>


        <div className="md-music-player">
          <div className="md-play-box">
            <div className={`fas fa-shuffle md-play-shuffle ${isShuffle ? 'play-shuffle-active' : ''}`} onClick={toggleShuffle}></div>
            <div className={`fas fa-repeat md-play-repeat ${isRepeat ? 'play-repeat-active' : ''}`} onClick={toggleRepeat}></div>
            ...
            <div className="fas fa-heart md-play-heart"></div>
            <div className="fas fa-plus-circle md-play-volume-up"></div>
          </div>
        </div>
